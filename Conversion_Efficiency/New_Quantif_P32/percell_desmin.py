"""CONVERSION EFFICIENCY v3 -- per-CELL Desmin positivity. PLATE_32.
Identical code/parameters to the other plates; only the nuclei directory differs
(plate32_nuclei).

One SHARED threshold (Otsu on the pooled distribution) splits positive from negative
across all wells -- no per-image normalisation, no per-well tuning, no calibration.

Run from Conversion_Efficiency/:
    cpenv/Scripts/python.exe New_Quantif_P32/percell_desmin.py [--ring-um 10]
"""
...
def main():
    ...
    for w in wells:
        ...
        ax[1].hist(v, bins=120, histtype="step", lw=1.3,
    ...
    fig.suptitle("PLATE_32 per-cell Desmin positivity — conversion efficiency v3",
                 fontweight="bold")
    fig.tight_layout()
    ...
    print("\n-> New_Quantif_P32/percell_desmin.png / .json")
