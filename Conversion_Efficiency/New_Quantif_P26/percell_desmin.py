...
PLATE_26. Identical code/parameters to New_Quantif_P23/percell_desmin.py; only the
nuclei directory differs (plate26_nuclei).
...
Desmin is cytoplasmic, so a converted cell has a bright ring. One SHARED threshold
(Otsu on the pooled distribution) splits positive from negative across all wells --
no per-image normalisation, no per-well tuning, no calibration to any expected value.

Run from Conversion_Efficiency/:
    cpenv/Scripts/python.exe New_Quantif_P26/percell_desmin.py [--ring-um 3]
"""
...
def main():
    ...
    fig.suptitle("PLATE_26 per-cell Desmin positivity — conversion efficiency v3",
                 fontweight="bold")
    fig.tight_layout()
    ...
    print("\n-> New_Quantif_P26/percell_desmin.png / .json")
