"""Visualise the TRUE per-cell conversion result for PLATE_26 so it can be eyeballed.
Identical code/parameters to New_Quantif_P23/visualize_final.py; only the well list
(ORDER) and plate label differ. A load-or-compute fallback builds the ring-value
cache if it is not present (P23 got it from ring_sweep; here it is self-contained).
...
RING_UM = 10.0
ORDER = ["23_B02_ctrl", "19_B06_actv104_trka", "32_C08_br223_igf1r"]
...
def main():
    ...
    ax.set_title("PLATE_26 conversion efficiency — per-cell Desmin+ "
    ...
    with open(os.path.join(HERE, "visualize_final.json"), "w") as fh:
        json.dump({"plate": "PLATE_26", "ring_um": RING_UM, "threshold_raw": thr,
