"""CONVERSION EFFICIENCY v3 -- per-CELL Desmin positivity (not pixel overlap).

Why v2 was not enough: thresholding a Desmin mask and asking "does the nucleus
overlap it" conflates two different questions -- how much Desmin territory exists,
and whether a given cell is Desmin+. That forces a trade-off: a threshold loose
enough to give the control its expected ~20% collapses the treated fold-change,
and one tight enough for 2-3x folds pushes the control to ~6%.

Conversion efficiency is a per-cell property, so measure it per cell:
  for every nucleus, take a CYTOPLASMIC RING around it (dilate the nucleus label,
  subtract the nucleus) and record the mean background-subtracted Desmin there.
  Desmin is cytoplasmic, so a converted cell has a bright ring; an unconverted
  one does not. One SHARED threshold (Otsu on the pooled distribution) then splits
  positive from negative across all wells -- no per-image normalisation anywhere.

This removes the dependence on mask morphology, dilation and overlap fraction
entirely, and the pooled per-cell distribution can be inspected for bimodality,
which tells us whether a clean split even exists.

Uses the cached top-hat images from conversion_v2.py, so it is fast.

Run from Conversion_Efficiency/:
    cpenv/Scripts/python.exe NEW_Quantif/percell_desmin.py [--ring-um 3]
"""
...
def main():
    ...
    fig.suptitle("Per-cell Desmin positivity — conversion efficiency v3",
                 fontweight="bold")
    fig.tight_layout()
    ...
    print("\n-> NEW_Quantif/percell_desmin.png / .json")
